...
export default async function GuruByKelasPage({ params }: Props) {
  if (params.slug[1] !== "page" && params.slug[1] !== undefined) {
    notFound();
  }

  const { data: guruData, error: guruError } = await getGuru(
    params.slug[2] || "1",
    params.slug[0]
  );
    ...
    <div className="mx-auto max-w-6xl my-16">
      ...
      <div className="mt-10">
        <TableComponent
          ...
          pathPagenation={`/dashboard/guru/kelas/${params.slug[0]}`}
          path="/dashboard/guru"
        />
      </div>
    </div>
