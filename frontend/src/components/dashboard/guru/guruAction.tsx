...
import useGuruAction from "@/hooks/dashboard/guru/useGuruAction";

interface Props {
  guru_id: string;
}

export default function GuruAction({ guru_id }: Props) {
  const { showDeleteModal, deleteButton, onCancelCallBack, onDeleteCallBack } =
    useGuruAction(guru_id);
  return (
    <>
      <div className="flex flex-row items-center gap-2 flex-wrap">
        <Link
          href={`/dashboard/guru/${guru_id}/edit`}
